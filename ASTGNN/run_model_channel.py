...
#
    print(encoder_inputs.shape, predict_output.shape, labels.shape)
    from matplotlib import pyplot as plt
    num_predicted_features = predict_output.shape[-1]
    sample_idx = 0
    adj = adjs[adjidx[sample_idx]]
    sample_input  = encoder_inputs[sample_idx] # (n_cell,n_time,n_feature)
    sample_output = predict_output[sample_idx]
    sample_labels = labels[sample_idx]
#
    node_idx = np.linspace(0,len(sample_input)-1,5).astype('int')
    for i_cell in node_idx:
        ...
        for n,ax in zip(neighbors,axes):
            input  = sample_input[n].detach().cpu().numpy() # n_time x n_feature
            label  = sample_labels[n].detach().cpu().numpy() # n_time x 1
            output = sample_output[n].detach().cpu().numpy() # n_time x 1
            ax.plot(label,color='green',label='True')
            ax.plot(output,color='red',label='Pred')
            # for color,input_f in zip(['springgreen','blue','skyblue'],
            #                          input.transpose()):
            #     ax.plot(input_f,color=color,label='Sensors')
            ax.legend()
#
        plt.savefig(os.path.join(outdir,f"pred-{type}-cell{i_cell}.svg"))
        plt.close()
#
    n_node_plot = min(len(sample_input),20)
    node_idx = np.linspace(0,len(sample_input)-1,n_node_plot).astype('int')
    ...
    for n,ax in zip(node_idx,axes):
        ...
        ax.legend()
#
    plt.savefig(os.path.join(outdir,f"pred-{type}-all.svg"))
    plt.close()
...
samples_dir = '../data/Devan/3channel/data_processed/EGF/samples.npz'
samples_dir = '../data/Devan/3channel/data_processed/EGF/scale-True/samples.npz'
...
points_per_hour = 75 # Time in
num_for_predict = 75 # Time out
num_for_predict_ = 10     # Time out
dataset_name = '3channel/EGF/scale-T'
model_name = 'ASTGNN'
learning_rate = 1e-3
start_epoch = 0
epochs = 50
...
TE = False # Temporal encoding
aware_temporal_context = False

smooth_layer_num = 0
use_LayerNorm = True
residual_connection = True

outdir = f'../out/test12/{dataset_name}'
os.makedirs(outdir,exist_ok=True)

# for adjtype in adj_name:
for adjtype in ['identity','dist-120']:
...
# for adjtype in ['dist-0','dist-120','neighbor-10','dist-60','neighbor-5']:
    ...
    adjs = adjs_all[:,i_adj]
    for idx_feature in [0,1,2]: # heldout sensor
        for setup in [0,1,2]:
                    ...
                    for num_layers in [4]:
                        for nb_head in [8]:
                            ...
                            for t_shift in [8]:
                                ...
                                #### Create dataset
                                dataout = create_data_loaders_channels(samples, batch_size,
                                ...
                                points_per_hour = train_target_tensor.shape[-1] # Time in
                                num_for_predict = points_per_hour # Time out
                                ...
                                if os.path.isfile(model_filename):
                                    ...
                                    #
                                        params_filename = os.path.join(params_path, 'epoch_best.params')
                                    #
                                    ...
                                    #
                                            ...
                                            optimizer.zero_grad()
                                            adjidx = adjidx.to(torch.int).cpu().numpy()
                                            outputs = net(encoder_inputs, adjidx)
                                    #
